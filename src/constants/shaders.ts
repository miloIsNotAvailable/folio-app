...
void main() {
    ...
    vec3 eye = vec3(0.1,1.41,(18.995));
    vec3 rd = rayDirection(10., u_resolution, gl_FragCoord.xy);
    
    mat4 view_world = viewWorldMatrix(eye, vec3(eye.x, eye.y, 0.), vec3(0., 1., 0.));
    vec3 view_dir = vec3(view_world * vec4(rd, 0.)).xyz;
    
    float dist = ray_march(eye, rd, 0., 10000. );

		if (dist > MAX_HIT_DIST - EPSILON) {
        vec3 c1 = vec3(1.);
        vec3 c2 = vec3(1., .85, .79);
        vec3 c3 = mix(c2, c1, distance(st, vec2(1., 0.)));
		gl_FragColor = vec4(c2, 1.);
		return;
		}
    
    vec3 p = eye + dist * view_dir;
    
    vec3 K_a = vec3(0.2);
    vec3 K_d = vec3(1., .88, .88);
    vec3 K_s = vec3(1.);
